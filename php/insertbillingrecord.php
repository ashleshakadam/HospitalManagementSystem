...
$sql ="SELECT * FROM billing WHERE appointmentid='$_GET[appid]]'";
...
if(mysqli_num_rows($qsql) == 0)
{
	...
	$sql = "INSERT INTO billing( patientid, appointmentid, billingdate, billingtime, discount, taxamount, discountreason, discharge_time, discharge_date) VALUES ('$_GET[patientid]','$_GET[appid]','$dt','$tim','0','0','','','')";
	$qsql=mysqli_query($con,$sql);
	//$billid = mysqli_insert_id($qsql);
}
else
{
	$billid = $rsbill[0];
...
if($billtype == "Doctor Charge" && $billtype1="Treatment Cost")
{
	...
	$sqlconsu= "SELECT * FROM billing_records WHERE billingid='$_GET[appid]' AND bill_type_id='$doctorid' AND bill_type='Consultancy Charge'";
	$qsqlcunsu = mysqli_query($con,$sqlconsu);
	
	if(mysqli_affected_rows($con) == 0)
	{
		$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status) VALUES ('$_POST[select2]','$doctorid','Consultancy Charge','$rsdoctor[consultancy_charge]','$dt','Active')";
		$qsql=mysqli_query($con,$sql);
	}
	...
	$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status) VALUES ('$_POST[select2]','$treatmentid','Treatment','$rstreatment[treatment_cost]','$dt','Active')";
	$qsql=mysqli_query($con,$sql);
}
...
if($billtype == "Prescription charge")
{
	...
	$rstreatment = mysqli_fetch_array($qsqltreatment);
	//Prescription charge
	 $sql ="INSERT INTO billing_records(billingid, bill_type_id, bill_type, bill_amount, bill_date, status) VALUES ('$_POST[select2]','$prescriptionid','Prescription Charge','$presamt','$dt','Active')";
	$qsql=mysqli_query($con,$sql);
}
...
{
//Consultancy Charge
$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status) VALUES ('$_POST[select2]','$doctorid','Consultancy Charge','$billamt','$dt','Active')";
$qsql=mysqli_query($con,$sql);
}
...
if($billtype == "Service Charge")
{
	...
	//Prescription charge
	$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status) VALUES ('$_POST[select2]','$servicetypeid','Service Charge','$servicecharge','$_POST[date]','Active')";
	$qsql=mysqli_query($con,$sql);
